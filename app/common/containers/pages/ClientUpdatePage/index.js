...
import { onDeleteClient, onSearchUsers } from './redux';
...
@provideHooks({
  fetch: ({ dispatch, params: { id } }) =>
    dispatch(fetchClientByID(id)).then(action =>
      Promise.all([
        dispatch(getUserByID(action.payload.entities.clients[id].user_id)),
        dispatch(fetchClientTypeByID(action.payload.entities.clients[id].client_type_id)),
      ])
    ),
})
...
}), ({ updateClient, onDeleteClient, onSearchUsers }))
export default class ClientUpdatePage extends React.Component {
  render() {
    ...
    const client_type_id = {
      ...
      title: clientTypes.filter(i => i.id === client.client_type_id)[0].name,
    };
    return (
      <FormPageWrapper id="client-update-page" title={t('Update client')} back="/clients">
        ...
        <ClientForm
          ...
          initialValues={{
            ...client,
            priv_settings: JSON.stringify(client.priv_settings),
            user_id,
            client_type_id,
          }}
